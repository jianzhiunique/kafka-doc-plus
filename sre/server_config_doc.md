...
为了解决上述问题，建立了对授权令牌的支持，以保护kafka。授权令牌是kafka服务器与客户端之间共享的秘密，因此无需通过KDC即可进行身份验证。
...
- 当`process.roles`不为空时，`node.id`与程序扮演的角色相关联，在KRaft下运行时，这是必需的配置。
